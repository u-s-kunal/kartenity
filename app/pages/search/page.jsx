...
import { setProducts } from "../../main/productSlice"; // adjust the path if needed
...
export default function SearchResults() {
  ...
  // Load products from localStorage on first mount
  useEffect(() => {
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
          {filtered.map((product) => {
            ...
            return (
              <div key={product._id} className="bg-white rounded-lg shadow-md p-3 flex flex-col items-center">
                {product.image && (
                  <img
                    src={product.image}
                    alt={product.name}
                ...
                <h2 className="text-lg font-semibold text-center line-clamp-1">{product.name}</h2>
